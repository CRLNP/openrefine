---
title: "OpenRefine"
author: "Caroline Patenaude</br>Biblithèque des lettres et sciences humaines, Université de Montréal</br>caroline.patenaude@umontreal.ca"
date: "`r Sys.Date()`"
output:
  rmdformats::robobook:
    self_contained: true
...
- Manipuler et tranformer des données.
- S'initier à l'usage du langage GREL.
</br>
</br>
</br>
</br>
...
- Le nettoyage de fichiers de données est une tâche souvent longue et ennnuyeuse mais incontournable avant toute analyse ou diffusion de données.
...
- Permet de facilement identifier et de corriger en lot des données incomplètes, manquantes, erronnées, incohérentes, non pertinentes, mal formattés, qu'elles soient numériques ou textuelles => sauve temps et énergie.
...
- *Nettoyer* : reprérer des erreurs, modifier le contenu des cellules...
...
- Associe la facilité d'usage d'un logiciel comme excel à la la puissance d'outils de programmation et d'algorithmes.
...
- **Reproductibilité**! Permet de garder un historique de toutes les manipulations effectuées sur ses données afin de les partager, de les réutilisées sur d'autres données...
...
- Créer en langage Java [donc nécessite l'installation de Java (64 bits de préférence)](https://www.java.com/en/download/).

- Préférable de définir Chrome ou Firefox comme navigateur par défaut (ne fonctionne pas dans IE).  
...
- Pour démarrer le logiciel, cliquer sur le fichier `` `openrefine.exe` `` ou `refine.bat`. Il est utile de faire un clic-droit sur le fichier .exe pour l'épingler dans sa barre de tâches (Sur Mac, déplacer l'icone OpenRefine dans le dossier d'applications et double cliquer sur celle-ci).

- Une fenêtre de commande noire s'ouvrira et lancera l'ouverture d'un onglet dans le navigateur web.  

- La fenêtre de commandes peut ensuite être ignoré mais NE PAS LA FERMER! Elle ne servira qu'à fermer la session de travail.  
...
- Repose donc sur deux composantes: un **seveur web local** et un **onglet de navigateur web** pointant vers l'adresse du serveur sur son poste.
...
- Le projet est enregistré en format OpenRefine automatiquement dans le dossier de travail chaque 5 minutes ([peut être modifié](https://docs.openrefine.org/manual/running#jvm-preferences)).
...
- Travaille avec 1GB de mémoire par défaut. À plus de 100 000 lignes, il peut être préférable d'[augmenter la mémoire](https://docs.openrefine.org/manual/installing#increasing-memory-allocation) de travail:  
...
<div class = "box1">
- Créer un nouveau projet en important le fichier `Incidents métro.csv` disponible sur le site [Données ouvertes Montréal](https://donnees.montreal.ca/societe-de-transport-de-montreal/incidents-du-reseau-du-metro) (voir aussi le site de la STM [Indicateurs de performance reliés au service métro](http://www.stm.info/fr/a-propos/informations-entreprise-et-financieres/experience-client/indicateurs-de-performance-relies))

- Explorer les options d'importation -> Porter une attention particulières aux options: `Supprimer les espaces` (8 -> cocher), 	`Utiliser le caractère	" pour fermer les cellules contenant les séparateurs de colonnes` (10 -> cocher ou non), `Analyser le texte des cellules comme nombres , dates…` (11 -> décocher)
...
1. Lien sous l'icône renvoit vers la page d'accueil/importation.
...
- Il est possible d'éditer manuellement le contenu de cellules individuelles en se placant sur chacune et en cliquant `edit`, mais OpenRefine privilégie le **travail en lot par colonnes** pour filtrer, trier, transformer.
...
- Pour qu'une cellule soit reconnu comme date, elle sera transformée au format *ISO-8601-compliant*.
...
<div class = "note">
- À l'importation d'un fichier, chaque cellule se vera attribuer un format. 
- Par défaut, le contenu des cellules est de format chaînes.  
- Les cellules étant reconnu comme numérique et date seront de couleur verte.  
...
- La sélection de lignes peut se faire par le biais de filtres, facettes et autres manipulations personnalitées.  
- Il est aussi possible de marquer/étoiler des lignes en lot ou individuellement puis de créer une facette sur la colonne `Toutes`.  
...
- Facettes et filtres s'enregistrent grace au `permalien`.  
...
2. Avec le **langage GREL** permettant de rédiger ses propres [expressions.](https://docs.openrefine.org/manual/expressions).  
...
## 6.1. Fonctions automatisées:

- Le menu et l'ensemble de ses fonctions automatisées apparait en cliquant sur la flèche de l'intitulé de chaque colonne.
...
GREL - Expressions similaires aux formules excel mais qui s'appliquent aux colonnes, non aux cellules, et qui ne sont pas stoquées à même les données mais dans un script.  

</br>

### Le langage GREL peut s'utiliser de différentes façons:  

- En créant des facettes personnalisées : `Facette -> Facette textuelle personnalisé`.  
...
- Quelle est la dernière date d'incident enregisrée dans le fichier ?  
- Supprimer le tri.  
</div>
</br>

<div class = "box1">
- Trier les valeurs de la colonne `Mois Calendrier`  
- Trier les valeurs de la colonne `Jour du mois`  
...
- La fonction `Filtrer le texte` permet de chercher une ou des chaînes de caractères dans une ou plusieurs variables.  
...
- Filtrer le texte sur la colonne *Heure de l'évenement*: "12"  
- Explorer les résultats  
- Comment s'assurer d'avoir seulement les cas survenus entre 12h et 13h? Combien de lignes correspondent à ces conditions? 
- Tout supprimer les filtres.
</div>
...
- À la base, une facette correspond à la liste des valeurs possibles d'une variable  (et de leur occurence) présentée selon le format (chaine, numérique, date).  
...
### Deux types de facettes:
  ...
  - [1] Les facettes automatisées. 
  </br>
  - [2] Les facettes personnalisées.
  </br>
  - [3] Il est aussi possible de créer ses propres facettes grâce au langage GREL pour définir des conditions de sélection/filtre très précises (on parle encore ici de `facettes personnalisées`, mais elles doivent être rédigées manuellement).

</br>

### Trois formats de facettes:

On retrouve également différents **formats de facettes** -> une pour chaque format de données  
...
- Les facettes personnalisées proposent aussi des facettes booléennes.  
...
- Permettent de combiner différents filtres en sélectionnant différentes valeurs d'une ou plusieurs variables (et leurs cas correspondants).  
  - Cliquer `inclure` ou `exlure` pour sélectionner ou supprimer des choix multiples **[12]**.  
  - Une fois la sélection faite, l'option `inverser` permet de sélectionner tout SAUF la sélection **[11]**.  
  - Peuvent être triées selon le nom ou le compte (nombre d'occurence).  
...
- À quel *jour de la semaine* cette cause est-elle survenue le plus souvent?  
...
- Ces évenements ont-ils nécessité une *évacuation*?  
...
# 8. Les regroupements (clustering)
...
- OpenRefine propose des *clusters* selon différentes méthodes plus ou moins élaborées (*Proche voisin* prend plus de mémoire) qui permettent de regrouper les valeurs similaires et proposent des matchs possibles (*fuzzy matching*): par exemple *empreinte* est plus conservateur (distingue casse, espaces, ordre, répétion, ponctuation...), *ngram* est plus flexible (même caractères), *Metaphone3* est le plus "libéral" (même prononciation).
...
# 9. GREL et les facettes personnalisées 
...
- Chaque caractère composant une valeur est ordonné dans un index **à partir de 0** (1er élément) et peut être ciblé grâce aux `[ ]`.
- De la même façon, chaque élément d'un *array* est ordonné dans un index **à partir de 0** (1er élément) et peut être ciblé grâce aux `[ ]`.  
...
Éditer les cellules -> Transformer -> `value.split(/\n/).slice(1).join("; ")` 
...
Créer une facette pour identifier les cas dont la valeur dans le colonne *Cause primaire* est la même que dans la colonne *Cause secondaire*
...
## 10.1 Créer et copier des colonnes
...
## 10.2. Diviser le contenu de cellules multivariées
...
Diviser par les longueurs de champs la colonne `Numéro d'incident` pour en créer deux nouvelles:  
  1. Une colonne contenant le premier catactère indiquant le type d'incident, soit T (train) ou S (service).  
...
## 10.3. Fusionner le contenu de plusieurs colonnes
...
<div class = "note">
Lorsque l'on fusionne des colonnes, il faut porter une attention particulière au séparateur choisi. Ne pas utiliser un caractère qui pourrait faire parti du contenu (ex: ,) ou qui servira à diviser les colonnes (.csv)
</div>
...
# 11. Les expressions régulières {#expression}
...
- Elles peuvent être utilisés à différents endroits dans OpenRefine et doivent toujours être entourées de `/ /`: filtres, facettes, fenêtre GREL.
...
### Exercice: diviser les éléments de références bibliographiques en format APA
...
- Supprimer tout le contenu sauf les noms de familles: `value.replace(/(\s[A-Z].\s)|(\s[A-Z].,\s)|(et)|(,)/, " ")`.  
</div>
...
**Fichier original téléchargé de la [Banque mondiale](https://databank.banquemondiale.org/reports.aspx?source=world-development-indicators) en format .csv composé de 5 pays et 4 indicateurs à l'horizontal et 3 années à la verticale**  
...
# 13. Exporter un fichier et fermer OpenRefine
...
- Aller à la fenêtre noire du terminal et utiliser les touches CTRL + C jusqu'à ce qu'elle se frme d'elle-même (il est parfois nécessaire de répéter à plus d'une reprise). Le fichier sera enregistré une dernière fois et le logiciel sera  maintenant bel et bien fermé.
...
# 14. Pour faire durer le plaisir!
...
Diviser le contenu d'une cellules pour les réordonner 

- value.split(",").reverse().join(",")

- value.split(",").sort().join(",")

Diviser après les 3 premiers caractères et conserve le premier élément  

- value.splitByLengths(3)[0] 

Diviser selon le type de caratères et retenir le deuxième élément  
...
<div class = "box1">
Ajouter un zéro devant le contenu si donne une chaîne de 0 à 2 caractères  
...
- if(test, si text est vrai, si text est vrai)
...
- [Site et décumentation officiels](https://openrefine.org/)
