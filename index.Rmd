...
- Permet de facilement identifier et de corriger en lot des données incomplètes, manquantes, erronées, incohérentes, non pertinentes, mal formatés, qu'elles soient numériques ou textuelles => sauve temps et énergie.
...
GREL - Expressions similaires aux formules excel mais qui s'appliquent aux colonnes, non aux cellules, et qui ne sont pas stockées à même les données mais dans un script.  
...
- Permettent de combiner différents filtres en sélectionnant différentes valeurs d'une ou plusieurs variables (et leurs cas correspondants).  
  ...
  - Peuvent être triées selon le nom ou le compte (nombre d'occurrence).  
...
- Ces événements ont-ils nécessité une *évacuation*?  
...
- Ici la fonction `slice()` conserve les éléments à partir de l'élément en position 1 donc supprime le 0. 
...
La syntaxe est composée d'éléments permettant de  cibler des caractères précis et de définir leur emplacement et leur occurence.
...
+-------------------------------------------------------+--------------------------------------------------+
| `()` : Grouper un contenu et le garder en mémoire      | `$` : Fin d'une ligne ou rappeller un groupe    |
+-------------------------------------------------------+--------------------------------------------------+
|  `\` : Chercher un caractère spécial littéralement    | Sans quantificateur, l'occurence est de 1       |
+-------------------------------------------------------+--------------------------------------------------+
